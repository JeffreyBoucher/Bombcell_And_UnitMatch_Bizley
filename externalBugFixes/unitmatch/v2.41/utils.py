...
def load_good_waveforms(wave_paths, unit_label_paths, param, good_units_only = True,splitShankActive=False,channel_pos=None,shankSplitSubset=None):
    """
    ...
    #see if bombcell unit labels
        if os.path.split(unit_label_paths[0])[1] == 'cluster_bc_unitType.tsv':
            ...
            tmp_idx = np.argwhere(np.isin(unit_label[:,1],['GOOD','MUA']))
        else:
            unit_label = load_tsv(unit_label_paths[i])
    ...
    for i in range(n_sessions):
        if splitShankActive:
            waveforms_reduced = waveforms[i]
            from UnitMatchPy.param_functions import get_max_sites
            fakeparam = param
            fakeparam['n_units'] = waveforms[i].shape[0]
            fakeparam['n_channels'] = waveforms[i].shape[2]
            fakeparam['n_units_per_session'] = [waveforms[i].shape[0]]
            clus_info = {
                'good_units': good_units[i],  # modified to Good and MUA
                'session_switch': [0],
                'session_id': [0]*fakeparam['n_units'],
                'original_ids': good_units[i]
            }
            
            max_site,_,_,_ = get_max_sites(waveforms_reduced, [channel_pos[i]], clus_info, fakeparam)
            subsetted = shankSplitSubset[max_site[:, 0]]
            waveforms_reduced = waveforms_reduced[subsetted,:,:,:]
            if i == 0:
                waveform = waveforms_reduced
            else:
                waveform = np.concatenate((waveform, waveforms_reduced), axis = 0)
            good_units[i] = good_units[i][subsetted]
        else:
            if i == 0:
        ...
        if splitShankActive:
            n_units_per_session[i] = waveforms_reduced.shape[0]
        else:
...
def paths_from_KS(KS_dirs, custom_raw_waveform_paths, custom_bombcell_paths, motion_week, motion_sessions, week_session_correspondance, do_channel_correction=False):
    """
    ...
    for i in range(n_sessions):
        ...
        if do_channel_correction:
            pos_tmp = correct_motion_on_channel_pos(pos_tmp,  motion_week, motion_sessions, i, week_session_correspondance)
    ...
    if week==0 and not(session==0):
        ...
        for i in range(week):
            ...
            for j,pos in enumerate(y_positions_init):
                ...
                new_positions.append(pos + local_motion)
            y_positions_init= new_positions.copy()
        for i in range(week_sessions,session+1):
            new_positions = []
            motion_df = motion_sessions[motion_sessions['session'] ==i]
            for j,pos in enumerate(y_positions_init):
                bins_center= np.array(motion_df['center_space_bin'])
